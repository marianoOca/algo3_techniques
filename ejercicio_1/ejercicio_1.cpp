...
/* -- Funciones de comparación para pairs -- */
struct {
...
/* -- Parametros de la funcion -- */
    ...
    // ordenamos K segun la influencia
    string orden = argv[1];
    if (orden == "menor_mayor") stable_sort(K.begin(), K.end(), cmp_menor_mayor);
    if (orden == "mayor_menor") stable_sort(K.begin(), K.end(), cmp_mayor_menor);
...
/* -- couts para chequear que estamos leyendo bien -- */
    ...
    //     for (int j = 0; j < E[0].size(); j++) cout << E[i][j] << " ";
    //     cout << endl;
    // }
    // for (int i = 0; i < K.size(); i++) cout << "(" << K[i] << "," << p[K[i]] << ")" << endl;
